...
import SRLH.SRLH_Agent_sim as Agent
import utils.calc_hr as calc_hr
from utils.DC_load_data import load_dataset
import SRLH.SRLH_Env_random_sim as Environment


parser = argparse.ArgumentParser (description="SRLH demo exp")
...
parser.add_argument('--exp_gamma', default=0.8, type=float,
                    help='hyper-parameter: discount factor (default: 1.0)')
parser.add_argument('--train_ndcg', type=int, default=100,
...
def test(model, nums, dsets, labels, code_length):
    ...
    testloader = DataLoader(dset_test, batch_size=opt.batch_size,
                            shuffle=False,
                            num_workers=4)
    qB, ind_qb = get_codes(model, testloader, num_test, code_length, opt.sparsity)
    rB_sy, ind_rb = get_codes(model, retrievalloader, num_database, code_length, opt.sparsity)
    ...
    return qB, rB_sy, map_sy, top_map_sy, Pres_sy, ndcg_sy, ind_qb, ind_rb
...
def SRLH_algo(code_length, dataname, date):
    ...
    for epoch in range (epochs):
        ...
        for iteration, (train_input, train_label, batch_ind) in enumerate (trainloader):
            ...
            for batch_i in range (len(batch_ind)):
                per_env = Environment.Env (z_scores[batch_i,:], train_label[batch_i,:], eval_index, sparsity, num_samples, database_labels, code_length, opt.train_ndcg)
        ...
        if (epoch % 2) == 1:
            print(' ============================= Testing ===================================')
            qB, rB_sy, map_sy, top_map_sy, Pres_sy, ndcg_sy, ind_qb, ind_rb = test(model, nums, dsets, labels, code_length)
    ...
    record['ind_qb'] = ind_qb
    record['ind_rb'] = ind_rb
...
if __name__ == "__main__":
    ...
    logdir = '-'.join (['log/run_ori9', opt.dataname, date])
    _logging ()
    _record ()
