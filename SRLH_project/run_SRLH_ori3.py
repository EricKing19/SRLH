...
def adjusting_learning_rate2(optimizer, epo):
    ...
    optimizer.module.param_groups[0]['lr'] = lr * 10
    optimizer.module.param_groups[1]['lr'] = lr
    print ('learning rate is adjusted!')
...
def SRLH_algo(code_length, dataname, date):
    ...
    if optimizer_name == 'SGD':
        ...
        optimizer = optim.RMSprop (params, weight_decay=weight_decay, momentum=0.9)
    else:
...
if __name__ == "__main__":
    ...
    logdir = '-'.join (['log/run_ori2', opt.dataname, date])
    _logging ()
    _record ()
