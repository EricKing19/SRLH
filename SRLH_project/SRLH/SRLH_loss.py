...
class SRLHLoss (nn.Module):
    '''
    except Tr(BLF)
    '''

    def __init__(self, sparsity, code_length):
        super (SRLHLoss, self).__init__ ()
        self.sparsity = sparsity
        self.code_length = code_length

    def forward(self, F_batch, actions, rewards):
        ...
        loss = 0.5 * (reg_loss.sum ()) / (batch_size * self.code_length) + self.lambda_3 * L1_loss.sum () / (
                    batch_size * self.code_length) + self.lambda_2 * bla_loss.sum () / self.code_length + 0.25 * oth_loss / (
                           batch_size * self.code_length)
        return loss
