...
class Agent:
    ...
    def noise_gumbel(self, shape, eps=1e-20):
        U = torch.Tensor(shape).uniform_(0, 1).cuda()
        return -(torch.log(-torch.log(U + eps) + eps))

    def sel_action_gumbel(self, z_scores, candidate_set):
        probs = self.softmax_index(z_scores, candidate_set)
        logits = probs.log()
        y = logits + Variable(self.noise_gumbel(logits.size()))
        action_temp = np.argmax(y.data) * torch.ones(1, )
        action = Variable(action_temp.long()).cuda()

        return int(action.cpu().data.numpy()), logits.gather(-1, action)
    ...
    def sample_episode(self, env):
        ...
        for s in range(env.sparsity):
            action, log_prob = self.sel_action_gumbel(env.z_scores, env.candidate_set)
