...
class Agent:
    def __init__(self, eps, EPSILON, exp_gamma):
        self.eps = eps
    ...
    def softmax_index(self, z_scores, candidate_set, cur_s):
        sel_outputs = self.sel_sparsity (z_scores, candidate_set)
        sel_outputs = torch.cat (sel_outputs) / (self.exp_gamma**cur_s)
        # sel_outputs = torch.cat(sel_outputs)
        return F.softmax (sel_outputs, dim=0)

    def sel_action(self, z_scores, candidate_set, cur_s):
        probs = self.softmax_index (z_scores, candidate_set, cur_s)
        m = Categorical(probs)
    ...
    def sample_episode(self, env):
        ...
        for s in range(env.sparsity):
            action, log_prob = self.sel_action(env.z_scores, env.candidate_set, s)
            env.encode_codes (action, s)
            actions.append (env.candidate_set[action])
            reward, done = env.step (action, actions)
